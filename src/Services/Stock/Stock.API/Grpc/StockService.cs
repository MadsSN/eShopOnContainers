...
namespace Stock.API.Grpc
{
    ...
    {
        private readonly StockContext _catalogContext;
        private readonly StockSettings _settings;
        private readonly ILogger _logger;
        public StockService(StockContext dbContext, IOptions<StockSettings> settings, ILogger<StockService> logger)
        {
        ...
        {
            _logger.LogInformation("Begin grpc call StockService.GetItemById for product id {Id}", request.Id);
            if (request.Id <= 0)
            {
            ...
            var item = await _catalogContext.Stocks.SingleOrDefaultAsync(ci => ci.Id == request.Id);
        ...
        public override async Task<PaginatedItemsResponse> GetItemsByIds(StockItemsRequest request, ServerCallContext context)
        {
            ...
            var totalItems = await _catalogContext.Stocks
                .LongCountAsync();

            var itemsOnPage = await _catalogContext.Stocks
                .OrderBy(c => c.Name)
        ...
        private PaginatedItemsResponse MapToResponse(List<Stock> items)
        {
            return this.MapToResponse(items, items.Count(), 1, items.Count());
        }

        private PaginatedItemsResponse MapToResponse(List<Stock> items, long count, int pageIndex, int pageSize)
        {
            ...
            items.ForEach(i =>
            {
                var brand = i.StockBrand == null
                            ? null
                            : new StockBrand()
                            {
                                Id = i.StockBrand.Id,
                                Name = i.StockBrand.Brand,
                            };
                var catalogType = i.StockType == null
                                  ? null
                                  : new StockType()
                                  {
                                      Id = i.StockType.Id,
                                      Type = i.StockType.Type,
                                  };

                result.Data.Add(new StockItemResponse()
                {
        ...
        private async Task<List<Stock>> GetItemsByIdsAsync(string ids)
        {
            ...
            var items = await _catalogContext.Stocks.Where(ci => idsToSelect.Contains(ci.Id)).ToListAsync();

...
namespace Stock.FunctionalTests
{
    public class StockScenariosBase
    {
        public TestServer CreateServer()
        {
            var path = Assembly.GetAssembly(typeof(StockScenariosBase))
              .Location;
            ...
            testServer.Host
                .MigrateDbContext<StockContext>((context, services) =>
                {
                    var env = services.GetService<IWebHostEnvironment>();
                    var settings = services.GetService<IOptions<StockSettings>>();
                    var logger = services.GetService<ILogger<StockContextSeed>>();

                    new StockContextSeed()

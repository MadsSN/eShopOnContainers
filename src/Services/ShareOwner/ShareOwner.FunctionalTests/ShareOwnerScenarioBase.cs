...
using ShareOwner.API.Extensions;
...
namespace ShareOwner.FunctionalTests
{
    public class ShareOwnerScenariosBase
    {
        public TestServer CreateServer()
        {
            var path = Assembly.GetAssembly(typeof(ShareOwnerScenariosBase))
              .Location;
            ...
            testServer.Host
                .MigrateDbContext<ShareOwnerContext>((context, services) =>
                {
                    var env = services.GetService<IWebHostEnvironment>();
                    var settings = services.GetService<IOptions<ShareOwnerSettings>>();
                    var logger = services.GetService<ILogger<ShareOwnerContextSeed>>();

                    new ShareOwnerContextSeed()

...
namespace ShareOwner.API.Grpc
{
    public class ShareOwnerService : ControllerBase
    {
        private readonly ShareOwnerContext _catalogContext;
        private readonly ShareOwnerSettings _settings;
        private readonly ILogger _logger;
        public ShareOwnerService(ShareOwnerContext dbContext, IOptions<ShareOwnerSettings> settings, ILogger<ShareOwnerService> logger)
        {
        ...
        {
            _logger.LogInformation("Begin grpc call ShareOwnerService.GetItemById for product id {Id}", request.Id);
            if (request.Id <= 0)
            {
            ...
            var item = await _catalogContext.ShareOwners.SingleOrDefaultAsync(ci => ci.Id == request.Id);
            ...
            if (item != null)
            {
                return new ShareOwnerItemResponse()
                {
        ...
        public override async Task<PaginatedItemsResponse> GetItemsByIds(ShareOwnerItemsRequest request, ServerCallContext context)
        {
            ...
            var totalItems = await _catalogContext.ShareOwners
                .LongCountAsync();

            var itemsOnPage = await _catalogContext.ShareOwners
                .OrderBy(c => c.Name)
        ...
        private PaginatedItemsResponse MapToResponse(List<ShareOwner> items)
        {
            return this.MapToResponse(items, items.Count(), 1, items.Count());
        }

        private PaginatedItemsResponse MapToResponse(List<ShareOwner> items, long count, int pageIndex, int pageSize)
        {
            ...
            items.ForEach(i =>
            {
                var brand = i.ShareOwnerBrand == null
                            ? null
                            : new ShareOwnerBrand()
                            {
                                Id = i.ShareOwnerBrand.Id,
                                Name = i.ShareOwnerBrand.Brand,
                            };
                var catalogType = i.ShareOwnerType == null
                                  ? null
                                  : new ShareOwnerType()
                                  {
                                      Id = i.ShareOwnerType.Id,
                                      Type = i.ShareOwnerType.Type,
                                  };

                result.Data.Add(new ShareOwnerItemResponse()
                {
                    ...
                    ShareOwnerBrand = brand,
                    ShareOwnerType = catalogType,
        ...
        private async Task<List<ShareOwner>> GetItemsByIdsAsync(string ids)
        {
            var numIds = ids.Split(',').Select(id => (Ok: int.TryParse(id, out int x), Value: x));

            if (!numIds.All(nid => nid.Ok))
            {
                return new List<ShareOwner>();
            }

            var idsToSelect = numIds
                .Select(id => id.Value);

            var items = await _catalogContext.ShareOwners.Where(ci => idsToSelect.Contains(ci.Id)).ToListAsync();
